...
func InitRMQ(rmqAddr, exchange, routingkey string) (*RMQ, error) {
	...
	go func() {
		for {
			...
			if !rmq.UnSendMsgQueue.Empty() {
				err := rmq.dealUnSendMsgQueue()
				if err != nil {
					fmt.Println("[" + time.Now().Format("2006-01-02 15:04:05.000") + "]  " + err.Error())
				}
			}
		}
	}()
...
func (rmq *RMQ) Publish(msg []byte) error {
	...
	if err != nil {
		if err1 := ReconnectRMQ(rmq); err1 != nil {
			return fmt.Errorf("发送失败:%v; 重连失败:%v", err, err1)
		}
		return fmt.Errorf("发送失败:%v 重连成功", err)
	}
